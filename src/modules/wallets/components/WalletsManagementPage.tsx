...
import { Wallet as WalletIcon, CheckCircle2, Clock, Plus, Eye, Edit2, Trash2 } from "lucide-react";
...
export function WalletsManagementPage() {
  ...
  const apiStats = statsResponse ?? { totalBalance: 0, avgBalance: 0, pendingWallets: 0 };

  const stats: StatItem[] = useMemo(() => {
    return [
      { icon: WalletIcon,   value: `$${apiStats.totalBalance.toLocaleString()}`, label: t("stats.totalBalance"), iconColor: "#0ea5e9", iconBg: "rgba(14,165,233,0.12)" },
      { icon: CheckCircle2, value: `$${apiStats.avgBalance.toLocaleString()}`, label: t("stats.avgBalance"), iconColor: "#22c55e", iconBg: "rgba(34,197,94,0.12)"  },
      { icon: Clock,        value: apiStats.pendingWallets, label: t("stats.pendingWallets"), iconColor: "#f59e0b", iconBg: "rgba(251,191,36,0.12)" },
    ];
  }, [t, apiStats]);
