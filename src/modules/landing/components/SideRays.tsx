...
const SideRays = ({
  ...
  // started: init the WebGL context once, the first time we come into view.
  // visibleRef: afterwards, scrolling in/out only pauses/resumes rendering —
  // it never destroys/recreates the context (that churn causes scroll jank).
...
void main() {
  ...
  gl_FragColor = color;
      ...
      const loop = (t: number) => {
        ...
        // Off-screen: keep the loop alive but skip all GPU work
  ...
  }, [started, speed, rayColor1, rayColor2, intensity, spread, origin, tilt, saturation, blend, falloff, opacity]);
