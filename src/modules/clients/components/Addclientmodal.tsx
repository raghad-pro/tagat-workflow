...
//   isOpen:      boolean;
//   onClose:     () => void;
//   onSave:      (data: AddClientFormValues) => void;
//   isPending?:  boolean;
//   /** super_admin only — list of companies to assign the new client to */
...
//       // company_admin لا يرسل company_id (الـ backend يحدده تلقائياً)
//       ...(isSuperAdmin && data.company_id ? { company_id: data.company_id } : {}),
//     });
//   };
...
//                   Assign to Company
//                   <span className="ds-text-gray-200 ds-font-regular text-[12px] ms-1">(optional)</span>
//                 </Text>
//                 <div className="relative">
//                   <select
//                     id="company_id"
//                     {...form.register("company_id", { valueAsNumber: true })}
//                     className="w-full appearance-none rounded-xl ds-border-input-color ds-bg-form ds-text-sm ds-text-primary px-4 focus:outline-none focus:border-[var(--color-primary)] transition-colors"
...
//                       <path d="M3 5L7 9L11 5" stroke="var(--color-text-gray-200)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
//                     </svg>
//                   </div>
//                 </div>
//               </div>
...
interface AddClientModalProps {
  isOpen:       boolean;
  onClose:      () => void;
  onAdd:        (data: AddClientFormValues) => void;  // ← onAdd (اسم الـ page)
  isPending?:   boolean;
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  <select
                    ...
                    {...form.register("company_id", { valueAsNumber: true })}
                    className="w-full appearance-none rounded-xl ds-border-input-color ds-bg-form ds-text-sm ds-text-primary px-4 focus:outline-none focus:border-[var(--color-primary)] transition-colors"
                    style={{ height: "var(--input-height)", border: "1px solid var(--color-border-inputs)" }}
                  >
                    <option value="">— Select a company —</option>
                    {companies.map((c) => (
                      <option key={c.id} value={c.id}>{c.name}</option>
